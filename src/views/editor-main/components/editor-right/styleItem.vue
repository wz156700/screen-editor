        ...
        >
          ...
          >
            <template v-if="items.type === 'el-input-number'">
              <el-input-number
                v-model="formPublic[items.field]"
                @change="onChange(items.field)"
                :min="isNaN(items.min) ? items.min : -Infinity"
                :max="isNaN(items.max) ? items.max : Infinity"
              />
            </template>
            <template v-if="items.type === 'el-select'">
              <el-select
                v-model="formPublic[items.field]"
                @change="onChangeSelect(items.field)"
                style="width: 100%"
              >
                <el-option
            ...
            <template v-if="items.type === 'el-radio'">
              ...
              >
                <el-radio
                  :label="val.value"
                  size="large"
                  v-for="(val, index) in items.options"
                  :key="index"
                  >{{ val.label }}</el-radio
                >
              </el-radio-group>
              ...
              <el-tabs v-model="activeName" class="demo-tabs">
                ...
                >
                  <p v-for="ele in item.children">
                    <span style="font-size: 0.65rem; color: #fff"
                      >{{ ele.name }}:</span
                    >
                    {{ ele.field }}
                    <el-color-picker
                      v-model="formPublic[ele.field]"
                      @change="onChange(ele.field)"
                      show-alpha
...
watch(
  () => formPublic,
  (news) => {
    console.log("变化了，嘻嘻嘻hahha", news.value);
  },
  {
    deep: true,
    immediate: true,
